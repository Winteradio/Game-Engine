...
**데이터 지향 설계(DOD)**, **메모리 연속성(Memory Contiguity)**, **로우 레벨 최적화**를 핵심 철학으로 개발된 개인 게임 엔진 프로젝트입니다.
...
- **하이브리드 스토리지:** Archetype 패턴의 빠른 순회 속도와 Sparse Set의 유연한 엔티티 관리 장점을 결합한  
	독자적인 **'Node'** 기반 스토리지를 구현했습니다.
- **의존성 관리:** **위상 정렬(Topological Sort)**을 사용하는 `SystemManager`를 통해  
	시스템 간의 실행 순서를 자동으로 스케줄링합니다.
- **병렬 처리:** 멀티스레드 환경에서의 안전한 시스템 실행을 지원하도록 설계되었습니다.

### 2. 메모리 관리 (Memory Management)
범용 힙 할당(Heap Allocation)의 오버헤드와 단편화를 방지하기 위해 특수 목적의 할당자를 사용합니다.
- **커스텀 할당자:** **Stack**, **Pool**, **Linear (Frame)** 할당자를 구현하여 상황에 맞는 메모리 관리를 수행합니다.
- **스마트 포인터:** `std::shared_ptr`의 오버헤드를 줄이고 객체 수명 주기를 안전하게 관리하기 위한 Reference Counting 시스템을 직접 구현했습니다.

### 3. 컨테이너 라이브러리 (Container Library)
게임 개발에 최적화된 STL 호환 컨테이너 라이브러리입니다.
- **No STL in Core:** 성능에 민감한 코어 로직에서 표준 STL을 배제하고,  
	메모리 정렬(Alignment)과 캐시 지역성을 보장하는 커스텀 `Vector`, `HashMap`, `Set`을 사용합니다.
- **최적화:** 불필요한 분기 예측 실패와 메모리 재할당을 최소화하도록 설계되었습니다.

### 4. 런타임 리플렉션 (Runtime Reflection)
동적 타입 처리를 위한 C++17 기반 리플렉션 시스템입니다.
- **정적 분석:** SFINAE와 템플릿 메타프로그래밍(TMP)을 활용하여 컴파일 타임에 타입 정보를 수집합니다.
- **직렬화:** 리플렉션 정보를 바탕으로 게임 객체의 자동 직렬화/역직렬화(JSON/Binary)를 지원하여 씬(Scene) 저장 및 로드 기능을 제공합니다.
- **자동화:** 매크로와 비-타입 템플릿 인자(Non-type template parameter)를 활용하여 새로운 타입 등록 시 보일러플레이트 코드를 최소화했습니다.

### 5. 그래픽스 (Graphics & OpenGL)
- **API:** OpenGL 4.5 Core Profile을 사용합니다.
- **Windowing:** **WGL (Win32 API)**을 직접 사용하여 윈도우 생성 및 컨텍스트 제어 과정을 정밀하게 관리합니다.
- **RHI 아키텍처:** 하이 레벨 렌더러와 로우 레벨 API 호출을 분리하는 **RHI (Render Hardware Interface)** 구조를 채택했습니다.
